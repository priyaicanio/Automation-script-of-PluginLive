...
sendkeys(getApplicationStartDate(), startdate);
       ...
       /* click(getNextstartdatebutton());
        ...
        click(getApplicationEndDate());
        click(getNextenddatebutton());
        click(getNextenddatebutton());
        for (int i = 0; i < getListEnddate().size(); i++) {
            if (getListEnddate().get(i).getText().equals(enddate)) {
                getListEnddate().get(i).click();
            }
        }
    }

    public void checkboxInternship() {
        click(getJobTypeInternship());
    }

    public void checkboxFresher() {
        click(getJobTypeFresher());
    }

    public void checkboxExperienced() {
        click(getJobTypeExperienced());
    }

    public void radiobtnOnroll() {
        click(getEmploymentTypeOnroll());
    }

    public void radiobtnContract() {
        click(getEmploymentTypeContract());
    }

    public void radiobtnFulltime() {
        click(getEmploymentTypeFullTime());
    }

    public void radiobtnParttime() {
    ...
    //public void joblist(){
      //  click(jobDetails);
        //for (int i = 0; i < getListfunction().size(); i++)
        //{
          //  String listValues = getListfunction().get(i).getText();
